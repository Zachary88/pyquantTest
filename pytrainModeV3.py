...
# 数据加载与预处理
# 训练集
data_train = ak.futures_zh_minute_sina(symbol=train_type, period=train_period)
data_train["return"] = data_train["close"].pct_change()
...
def create_dataset(data, look_back=60):
    ...
    for i in range(look_back, len(data)):
        ...
        y.append(
            1 if (data[i, 3] > data[i - look_back, 3]) else 0
        )  # 比较当前close与look_back前的close
